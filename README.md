...
1. Paso 0
    ...
    > ¿Opciones más usadas?
    1. El operador sizeof() devuelve el tamanio en bytes del tipo de dato pasado o variable por parametro. El valor de salida de dicha funcion depende de la arquitectura de nuestra computadora. En mi caso al tener una de 64 bits,sizeof(char) devolveria un 1 mientras que sizeof(int) devolveria 4.
    > No sólo depende de la arquitectura, también del compilador
    ...
    > Bien, pero falta decir que ese espacio no utilizado es *padding* que se agrega para alinear los atributos. En este caso se alinea *c*.
...
1. Paso 4 
    ...
    > El error cambia: strncpy nos permite determinar hasta cuántos caracteres copiar. Si elegimos n <= al tamaño del buffer no tenemos buffer overflow, pero como no agrega el caracter nulo, tendríamos un invalid read. Así, si agregamos un caracter nulo al final de buffer no tendríamos el error, sólo nos quedaría el nombre de archivo truncado y el programa no lo abriría.
    ...
    > Segmentation fault se da cuando accedemos a un segmento de memoria restringida. La detección se hace por hardware (MMU) y el kernel envía una señal que mata al proceso. El buffer overflow no es detectado por hardware ni por el kernel, siendo más peligroso por ser "silencioso".
